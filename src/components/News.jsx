import React, { useEffect, useState } from 'react'
import { Select,Typography,Row,Col,Avatar,Card } from 'antd'
import { useGetCryptosNewsQuery } from '../services/cryptoNewsapi'
import moment from 'moment/moment'
import { useGetCryptosQuery } from '../services/Crypto'
const News = ({simplyfied}) => {
  const [news,setNews] = useState([])
  const count = simplyfied?10:100
  const {data:category} = useGetCryptosQuery(100)
const img = 'data:image/png;base64,'
const [newscategory,setCategory] = useState('crypto')
  const {data,isFetching} = useGetCryptosNewsQuery({x:newscategory,y:count})
  console.log(data?.value)
 const handelselect = (e)=>{
  console.log(category.data.coins)
  setCategory(e)
 }
if(isFetching) return 'Loading'
  return (
    <div>
      {!simplyfied && <select onChange={(e)=>handelselect(e.target.value)} >
          <option value="crypto">crypto</option>
         {category.data.coins.map((e)=>{
          return (<option value={e.name}>{e.name}</option>)
         })}
        </select>}
      {data?.value.map((e)=>{
        const {name,url,image,datePublished,description} = e
     
         return(
          
          <div className=""><a href={url}>
            <img src={image?.thumbnail?.contentUrl || img} alt="" />
            <p>{name}</p></a>
            <p>{moment(datePublished).startOf('ss').fromNow()}</p>
            <p title={description} >{description.slice(0,40)}...</p>
            </div>
         )
      })}
        News
      
    </div>
  )
}

export default News
